...
class Client {
  ...
  public:
	...
	void sendCmdValidation(const Message  inMessage,
						   const Channel &channel) const;
	bool sendMessageTo(Message msg, const std::string recipientNickname,
					   Server &server) const;
	void sendErrorMessage(MessageType				type,
						  std::vector<std::string> &args) const;
	void sendErrorMessage(MessageType type, std::string arg1) const;
	void sendErrorMessage(MessageType type, std::string arg1,
